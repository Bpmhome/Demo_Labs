...
Kurtis Mclain
02-27-2019
Testing using a struct
14-1 Performance lab.
*/
#include <stdio.h>
#include <string.h>
...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int  *errorCode_ptr)
{
	...
	//For loop to get the length of sentence_ptr
	for(int i = 0; i < strlen(sentence_ptr); i++)
    {
		//Keeps searchWord_ptr from being bigger then sentence_ptr
        if((strlen((sentence_ptr)) - strlen(searchWord_ptr)) >= 0)
        {
			//For loop to get the length of searchWord_ptr
            for(int j = 0; j < strlen(searchWord_ptr); j++)
            {
				//if statement to see if the word is in the sentence and if not break
